...
gff_NCBI_chr14 <- data.frame(gff_NCBI) %>%
  ...
  filter(seqnames == "NC_045165.1", end > 4040000, start < 4270000, type %in% c("gene", "mRNA", "pseudogene")) %>%
  mutate(seqnames = 14)
write.csv(gff_NCBI_chr14, "genes_chr14.csv", row.names = F)
...
genes_in <- read.csv("genes_chr14.csv", header=T)

genes <- genes_in %>% 
  filter(seqnames == 14, end > 4040000, start < 4270000, gene_biotype %in% c("protein_coding", "pseudogene"), type %in% c("gene", "pseudogene")) %>%
  mutate(start = ifelse(start < 4040000, 4040000, start)) %>%
  mutate(end = ifelse(end > 4270000, 4270000, end)) %>%
...
q <- ggplot(genes, aes(xmin = start/1000000, xmax = end/1000000, fill = gene_name_final, color = gene_name_final, y = 1, forward = strand)) +
  geom_gene_arrow() +
  scale_x_continuous(expand = c(0,0), limits = c(4.04, 4.27), breaks = seq(4.05, 4.25,.05)) +
  scale_y_continuous(expand = c(0,0), limits = c(0, 2)) +
  scale_color_manual(values = brewer.pal(n=8, name="Set1")[c(2,7,9,8,3,4)], limits = c("BTN2A2", "Uncharacterized", "Pseudogene", "zgc:162509", "RAB3GAP2", "IARS2"), labels = c(expression(bolditalic("BTN2A2")), expression(bold("Uncharacterized")), expression(bold("Pseudogene")), expression(bolditalic("zgc:162509")), expression(bolditalic("RAB3GAP2")), expression(bolditalic("IARS2")))) +
  scale_fill_manual(values = brewer.pal(n=8, name="Set1")[c(2,7,9,8,3,4)], limits = c("BTN2A2", "Uncharacterized", "Pseudogene", "zgc:162509", "RAB3GAP2", "IARS2"), labels = c(expression(bolditalic("BTN2A2")), expression(bold("Uncharacterized")), expression(bold("Pseudogene")), expression(bolditalic("zgc:162509")), expression(bolditalic("RAB3GAP2")), expression(bolditalic("IARS2")))) +
  labs(x = "Chromosome 14 (Mb)", y = "Genes") +
...
diversity <- diversity %>% filter(chromosome == "chr14" & window_pos_1 > 4040000 & window_pos_2 <= 4270000)
...
png(filename="plot.png", width = 2000, height = 900, units = 'px',res=195) 
print(p)
dev.off()
